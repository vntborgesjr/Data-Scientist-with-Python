...
data = pulls.merge(right = pull_files, on = 'pid')
```
...
# Plot the results
counts.plot(kind = 'bar', figsize = (12, 4), xlabel = "Time", ylabel = 'Number of contributions')
```
...
by_user = data.groupby('user')['pid'].value_counts()

# Plot the histogram
by_user.plot(kind = 'hist', xlabel = 'Number of users')
```
...
last_10 = pulls.nlargest(n = 10, columns = 'date', keep = 'last')
...
print(author_counts.nlargest(n = 3))
```
...
users_last_10 = set(joined_pr.nlargest(n = 10, columns = 'date_x')['user_x'].unique())
...
# Plot the results
counts_wide.plot(kind = 'bar', xlabel = 'Year', ylabel = "Number of contribution")
```
...
# Plot the results
by_file_wide.plot(kind = 'bar', xlabel = 'Year', ylabel = 'Number of contributions')
```
